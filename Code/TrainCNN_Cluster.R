...
#install keras only if its first iter
if(iter==1){
install_keras()}else{
  while(is_keras_available()==FALSE){
    print("waiting for keras to install!")
  }
}
...
CNN %>% save_model_hdf5(paste("TrainedCNN", iter, sep=""))
...
save(predictions, file=paste("Prediction", iter,".rda",sep=""))
